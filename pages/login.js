...
const login = () => {
    ...
    const alreadyLoggedInRedirect = (event) => {
      event.preventDefault()
      router.push('/')
    }
    ...
    async function handleLogin(event) {
        ...
        await supabase.auth.signIn({ email: email, password: password }).then(res => {
            if (!res.error) {
            if (res) {
                router.push('/')
            } else {
...
<div class="min-h-screen flex flex-col items-center justify-center bg-gray-300">
  <div class="flex flex-col bg-white shadow-md px-4 sm:px-6 md:px-8 lg:px-10 py-8 rounded-md w-full max-w-md">
    ...
    <button class="relative mt-6 border rounded-md py-2 text-sm text-gray-800 bg-gray-100 hover:bg-gray-200">
      <span class="absolute left-0 top-0 flex items-center align-middle justify-center h-full w-10 text-2xl"><FcGoogle/></span>
      <span>Login with Google (Temporarily Disabled)</span>
    </button>
    ...
    <div class="flex justify-center items-center mt-6">
      <a href="#" target="_blank" class="inline-flex items-center font-bold text-blue-500 hover:text-blue-700 text-xs text-center">
        ...
        <span class="ml-2" onClick={signUpRedirect}>Don't have an account?</span>
      </a>
    </div>
  </div>
